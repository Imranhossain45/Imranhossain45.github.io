...
@endphp
@foreach ($scheduled_match_list_int as $matchList)
      ...
      <div class="block" style="background: #E9C4C4">
        <a href="{{ route('match', ['leagueID' => $match->seriesId, 'matchID' => $match->matchId, 'slug' => $slug]) }}">
          <div class="fx-top-blk">
            <h4>
              <a target="_blank" rel="noopener noreferrer" href="{{ route('match', ['leagueID' => $match->seriesId, 'matchID' => $match->matchId, 'slug' => $slug]) }}">{{ @$matchList->seriesName }}
          ...
          <div class="text-center">
            <a href="{{ route('match', ['leagueID' => $match->seriesId, 'matchID' => $match->matchId,'slug'=>$slug]) }}">View
              Match Details</a>
          </div>
